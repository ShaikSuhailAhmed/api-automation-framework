...
public class AllureLogs implements ITestListener {
    ...
    public static void softAssertFalse(SoftAssert softAssert, boolean condition, String message) {
        try {
            ...
            // Log the assertion failure if you want to show in Allure logs too
            AllureLogs.step("❌ " + message + " (Expected: false, but was true)");
        }
    }
    ...
    @Step
    public static void softAssertMissingParam(SoftAssert softAssert, String message) {
        softAssert.fail(message); // record failure but continue
        io.qameta.allure.Allure.step("❌ Missing Param: " + message);
    }
    ...
    // ✅ Add this method for NULL validation
    @Step
    public static void softAssertNull(SoftAssert softAssert, Object actual, String message) {
        Allure.step(message + " | Value: " + actual);
